...
setup_hailort_readonly_rootfs() {
    install -d ${IMAGE_ROOTFS}${sysconfdir}/profile.d
    cat > ${IMAGE_ROOTFS}${sysconfdir}/profile.d/hailort.sh << 'EOF'
# Avoid writes to /home/root/.hailo on read-only rootfs images.
export HAILORT_LOGGER_PATH=NONE
EOF

    install -d ${IMAGE_ROOTFS}${sysconfdir}/environment.d
    cat > ${IMAGE_ROOTFS}${sysconfdir}/environment.d/10-hailort.conf << 'EOF'
...
#
ROOTFS_POSTPROCESS_COMMAND += "set_ssh_login_policy; setup_wpa_supplicant_dir; setup_wifi_autoconnect; setup_camera_modules_load; setup_hailort_readonly_rootfs; setup_camera_module_service; setup_rootfs_autogrow; "

# Chargement automatique des modules au boot
# Les noms de modules utilisent des underscores (pas des tirets)
KERNEL_MODULE_AUTOLOAD += "imx477 bcm2835_unicam rp1_cfe pisp_be v4l2_async v4l2_fwnode v4l2_cci v4l2_mem2mem videobuf2_common videobuf2_dma_contig videobuf2_memops videobuf2_v4l2 brcmfmac brcmutil cfg80211 rfkill pwm_fan gpio_fan hailo_pci"



# Strip binaries and debug symbols to reduce image size
...
# Firmware is downloaded from AWS S3 and represents majority of image size
# Without Hailo: ~70 MB | With Hailo 8L: ~130 MB (firmware ~60 MB uncompressed)
